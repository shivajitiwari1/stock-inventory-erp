...
export const Sidebar: React.FC = () => {
  ...
  return (
    <aside className={`${isOpen ? 'w-64' : 'w-20'} bg-white dark:bg-slate-800 border-r border-gray-200 dark:border-slate-700 transition-all duration-300 fixed h-screen overflow-y-auto flex flex-col`}>

      {/* Header */}
      <div className="p-4 flex items-center justify-between border-b border-gray-200 dark:border-slate-700 shrink-0">
        {isOpen && <h1 className="text-xl font-bold text-gray-800 dark:text-slate-100">Stock ERP</h1>}
        <button
      ...
      <div className="p-4 border-t border-gray-200 dark:border-slate-700 space-y-2 shrink-0">
        ...
        {user ? (
          ...
          >
            ...
            {isOpen && <span className="text-sm">Login</span>}
          </button>
        )}
      </div>
